...
@section('content')
   ...
   @auth()
      <div class="bg-white border border-gray-300 p-8 rounded-lg shadow-lg w-50%">
         <h1 class="text-lg mb-5 text-center font-bold text-[#7C3AED]">My Todo List...</h1>
         {{-- Task Input Form --}}
         <form method="POST" class="flex mb-6" action="{{ route('tasks.store') }}">
            @csrf
            <input type="text" name="title" placeholder="Add a new task..."
                  class=" p-3 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500">
            <input type="text" name="description" placeholder="Add description..."
                  class=" p-3 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 ml-2"/>       
            <input type="submit" value="Add Task" class="bg-blue-500 text-white p-3 rounded hover:bg-blue-600 transition duration-200 ml-2" />
         </form>
         @error('title')   
            <p class="flex text-red-500 text-sm">{{ $message }}</p>
         @enderror
